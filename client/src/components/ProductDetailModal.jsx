...
import { X, Plus, ShieldCheck, Truck, Clock, AlertCircle } from 'lucide-react';
...
const ProductDetailModal = ({ product, onClose, onAddToCart }) => {
  ...
  const displayImage = product.image || (Array.isArray(product.images) && product.images[0]) || FALLBACK_SVG;
          ...
          <div className="fixed inset-0 z-[201] flex items-center justify-center p-4 pointer-events-none">
            ...
            >
              ...
              <div className="bg-surface-container-low h-60 flex items-center justify-center p-8 flex-shrink-0">
                <img
                  src={imgError ? FALLBACK_SVG : displayImage}
                  alt={product.name}
              ...
              <div className="p-6 space-y-4 overflow-y-auto">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  {isOutOfStock ? (
                    <span className="flex items-center gap-1.5 text-xs bg-red-50 text-red-700 border border-red-200 px-3 py-1.5 rounded-full font-bold">
                      <AlertCircle className="w-3.5 h-3.5 text-red-600" /> Out of Stock
                    </span>
                  ) : (
                    <span className="flex items-center gap-1.5 text-xs bg-amber-50 text-amber-800 border border-amber-200 px-3 py-1.5 rounded-full font-semibold">
                      <Clock className="w-3.5 h-3.5 text-amber-600" /> In Stock ({product.stock} available)
                    </span>
                  )}
                </div>
